...
db.serialize(() => {
  ...
  db.run(`CREATE TABLE IF NOT EXISTS teacher_reports (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    student_name TEXT,
    teacher_name TEXT,
    comment TEXT,
...
app.post("/api/register", (req, res) => {
  ...
  const code = req.body.code || req.body.regCode;
  const userRole = role ? role.toLowerCase() : 'alumno';
...
app.post("/api/student/report", (req, res) => {
  if (req.session.role !== 'alumno') {
    return res.status(403).json({ error: "Solo los alumnos pueden enviar opiniones o reportes." });
  }
  const { teacher_name, comment } = req.body;
  const student_name = req.session.name;

  db.run(`INSERT INTO teacher_reports (student_name, teacher_name, comment, date) VALUES (?, ?, ?, datetime('now'))`, 
    [student_name, teacher_name, comment], function(err) {
      if (err) return res.status(500).json({ error: "Error al guardar el reporte en la base de datos." });
      res.json({ success: true, message: "Opinión enviada y guardada correctamente para los administradores." });
  });
});
...
app.get("/api/admin/reports", (req, res) => {
  ...
  db.all(`SELECT * FROM teacher_reports ORDER BY date DESC`, [], (err, rows) => {
  ...
  try {
    if (query.includes("imagen") || query.includes("genera una imagen") || query.includes("foto") || query.includes("crear imagen")) {
      ...
      db.all("SELECT * FROM schedules", [], (err, rows) => {
        ...
        markdownTable += "| Día | Horario | Materia | Profesor | Estado / Salón |\n";
        markdownTable += "|---|---|---|---|---|\n";
        rows.forEach(r => {
          markdownTable += `| ${r.day} | ${r.start_time} - ${r.end_time} | ${r.subject} | ${r.teacher || 'Sin asignar'} | ${r.status} (${r.room}) |\n`;
        });
      ...
      if (currentRole !== 'profesor') {
        return res.json({ answer: "Esta función de solicitud de cobertura es exclusiva para profesores." });
      }
      ...
      res.json({ 
        answer: `Hola, ${currentTeacherName}. Como **${currentRole}**, puedes pedirme la cuadrilla escribiendo 'horario', consultar noticias o revisar los estados de disponibilidad.` 
      });
    }
